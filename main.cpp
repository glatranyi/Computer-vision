...
int main(void) {
	...
	if (!capVideo.isOpened()) {                                                 // ha nemlehet megnyitni videó fájlt
		...
		_getch();     // billentyûlenyomásra vár               
		return(0);                                                             
	}
	...
	while (capVideo.isOpened() && chCheckForEscKey != 27) {   //ameddig a videó meg van nyitva és nem ütöttek ESC-et
		...
		if (blnFirstFrame == true) { //elsõ képkockán
			for (auto &currentFrameBlob : currentFrameBlobs) {
				blobs.push_back(currentFrameBlob); //legyen mindegyik végleges Blob
			}
		}
		else {
			matchCurrentFrameBlobsToExistingBlobs(blobs, currentFrameBlobs); // meglévõ Blob-hoz matcheljük (melyik van legközelebb)
		}
		...
		imgFrame1 = imgFrame2.clone();           // elsõ képkockát a második helyére mozgatja
...
void matchCurrentFrameBlobsToExistingBlobs(std::vector<Blob> &existingBlobs, std::vector<Blob> &currentFrameBlobs) {
	...
	for (auto &currentFrameBlob : currentFrameBlobs) {  //a jelenlegi képkockán szereplõ Blob-okon végigmegyünk

		int intIndexOfLeastDistance = 0;
		double dblLeastDistance = 100000.0;

		for (unsigned int i = 0; i < existingBlobs.size(); i++) {  //a már létezõ Blob-okon végigmegyünk
		...
		if (dblLeastDistance < currentFrameBlob.dblCurrentDiagonalSize * 0.5) { //ha a current blob tresholdja(konvex burka átlósan *0,5) akkor
			addBlobToExistingBlobs(currentFrameBlob, existingBlobs, intIndexOfLeastDistance); //a meglévõhöz hozzáadjuk
		}
		else {
	...
	for (auto &existingBlob : existingBlobs) {   
		...
		if (existingBlob.blnCurrentMatchFoundOrNewBlob == false) {   // ha már megtaláltuk az összes Blob-ot ugrunk a következõ képkockára
			existingBlob.intNumOfConsecutiveFramesWithoutAMatch++;
		}
			
		if (existingBlob.intNumOfConsecutiveFramesWithoutAMatch >= 5) {   // biztos, hogy kiment a képrõl, nem követjük tovább
...
bool checkIfBlobsCrossedTheLine(std::vector<Blob> &blobs, int &intHorizontalLinePosition, int &carCount) {
	...
	for (auto blob : blobs) {
		...
		if (blob.blnStillBeingTracked == true && blob.centerPositions.size() >= 2) {
			...
			if (blob.centerPositions[prevFrameIndex].y > intHorizontalLinePosition && blob.centerPositions[currFrameIndex].y <= intHorizontalLinePosition) {  // blob pozíció az elõzõ képkockában alatta vagy fölötte lett
				carCount++;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
void drawCarCountOnImage(int &carCount, cv::Mat &imgFrame2Copy) { // kiiratás autóösszeg jobb felsõ
